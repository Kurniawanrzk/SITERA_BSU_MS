...
class checkifsuperadmin
{
    ...
    public function handle(Request $request, Closure $next): mixed
    {
        ...
        try {
            ...
            // Token valid, cek apakah user adalah superadmin
            if (!isset($authData['data']['roles']['is_superadmin']) || $authData['data']['roles']['is_superadmin'] !== true) {
                return $this->errorResponse("Akses ditolak. Anda bukan pengguna superadmin.", 403);
            }
            
            // Tambahkan user_id dan superadmin_id ke request untuk digunakan di controller
            $request->merge([
                'user_id' => $authData['data']['user']['id'] ?? null,
                'superadmin_id' => $authData['data']['roles']['superadmin_id'] ?? null,
